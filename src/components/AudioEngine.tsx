...
export default function AudioEngine() {
  ...
  const { settings, updateSettings, updateAudioTools, updateSyncLabSettings } = useSettings();
  ...
  const runHealthCheck = async () => {
    ...
    const isContinuityOn = adv?.stabilityEngine?.sessionContinuity?.isEnabled;
    const isHeatmapOn = adv?.stabilityEngine?.heatmapInsights?.isEnabled;

    if (!isPlayingRef.current && (!isEmergencyOn || !isForeground)) return;

    // F. Session Continuity & Heatmap Tracking
    const now = Date.now();
    if (isPlayingRef.current) {
       // Continuity: Throttled State Sync (5s)
       if (isContinuityOn && now - lastContinuitySave.current > 5000) {
          updateSettings({
            sessionState: {
              ...settingsRef.current.sessionState,
              lastActiveAt: now
            },
            advancedAudio: {
              ...settingsRef.current.advancedAudio,
              stabilityEngine: {
                ...settingsRef.current.advancedAudio.stabilityEngine,
                sessionContinuity: {
                  ...settingsRef.current.advancedAudio.stabilityEngine.sessionContinuity,
                  lastSavedAt: now
                }
              }
            }
       ...
       if (isHeatmapOn) {
          ...
          const currentHistory = settingsRef.current.advancedAudio.stabilityEngine.heatmapInsights.history;
          const todayStats = currentHistory[dateKey] || { duration: 0, usage: [] };
          
          if (now % 30000 < 5000) { // Every ~30s sync heatmap
            const activeFeatures: string[] = [];
            if (settingsRef.current.subliminal.isEnabled) activeFeatures.push('subliminal');
            if (settingsRef.current.binaural.isEnabled) activeFeatures.push('binaural');
            if (settingsRef.current.pureHz.isEnabled) activeFeatures.push('pureHz');
            if (settingsRef.current.nature.isEnabled) activeFeatures.push('nature');
            if (settingsRef.current.noise.isEnabled) activeFeatures.push('noise');
            if (settingsRef.current.didgeridoo.isEnabled) activeFeatures.push('didgeridoo');
            if (settingsRef.current.isochronic.isEnabled) activeFeatures.push('isochronic');
            if (settingsRef.current.solfeggio.isEnabled) activeFeatures.push('solfeggio');
            if (settingsRef.current.schumann.isEnabled) activeFeatures.push('schumann');
            if (settingsRef.current.shamanic.isEnabled) activeFeatures.push('shamanic');
            if (settingsRef.current.mentalToughness.isEnabled) activeFeatures.push('mentalToughness');
            if (isPlayingRef.current) activeFeatures.push('main');

            const newHistory = { ...currentHistory };
            newHistory[dateKey] = {
              duration: todayStats.duration + 30,
              usage: Array.from(new Set([...todayStats.usage, ...activeFeatures]))
            };
            updateSettings({
              advancedAudio: {
                ...settingsRef.current.advancedAudio,
                stabilityEngine: {
                  ...settingsRef.current.advancedAudio.stabilityEngine,
                  heatmapInsights: {
                    ...settingsRef.current.advancedAudio.stabilityEngine.heatmapInsights,
                    history: newHistory
                  }
                }
              }
    ...
    // B. Main Audio Stall Check (Self-Heal Engine)
    if (mainAudio && mainAudio.src && !mainAudio.ended) {
      ...
      // Detection Logic: 
      // 1. Unexpected Pause
      const isUnexpectedlyPaused = mainAudio.paused && isPlayingRef.current;
      
      // 2. Frozen Time (No progress for 2-3s)
      const isFrozen = isPlayingRef.current && !mainAudio.paused && 
                       (currentPos === lastKnownTime.current) && (currentPos !== 0);

      if (isUnexpectedlyPaused) {
        stallCount.current['main'] = (stallCount.current['main'] || 0) + 1;
        console.warn(`[Stability] Main Audio paused unexpectedly (${stallCount.current['main']}). Repairing...`);
        
        if (stallCount.current['main'] >= 1) {
           mainAudio.play().catch(async (e) => {
             console.warn('[Stability] First-tap play failed:', e);
        ...
        if (playStateAnomalies.current >= 3) { // ~4.5-9s frozen depending on health speed
          console.error('[Stability] Main Audio frozen (time locked). Initializing Self-Heal...');
          await softHealElement(mainAudio, 'main');
          playStateAnomalies.current = 0;
        }
      } else {
        // Reset counters if playing fine
        playStateAnomalies.current = 0;
        stallCount.current['main'] = 0;
    ...
    // C. Heartbeat stability - Keep session alive even if all layers pause
    // iOS 16 Root Fix: Process stays active as long as ANY audio is playing
    const isMasterOn = settingsRef.current.globalModes.masterAudioEnabled;
    const isSafeIdleOn = settingsRef.current.safeIdle.isEnabled;
    if (heartbeatAudioRef.current && (isMasterOn || isSafeIdleOn)) {
      if (heartbeatAudioRef.current.paused) {
        heartbeatAudioRef.current.play().catch(() => {});
      }
      // Ensure it stays at configured volume (inaudible but active)
      heartbeatAudioRef.current.volume = settingsRef.current.safeIdle.volume;
    }
    ...
    // G. Time-Based Automation
    const timeAutomation = adv?.calmControl?.timeAutomation;
    if (timeAutomation?.isEnabled && isPlayingRef.current && mainAudio) {
      const currentPos = mainAudio.currentTime;
      // Precision check: we check if we just passed a step
      const lastCheckPos = lastKnownTime.current;
      const step = timeAutomation.steps.find(s => 
        (s.time > lastCheckPos && s.time <= currentPos) || 
        (Math.abs(s.time - currentPos) < 0.5)
      );
      
      if (step) {
        console.log(`[Automation] Triggering step: ${step.type} -> ${step.value} at ${step.time}s`);
        if (step.type === 'volume') {
          updateSettings({ mainVolume: step.value });
        } else if (step.type === 'masterHz') {
          updateSyncLabSettings({ masterHz: step.value });
        }
      }
    }
  ...
  useEffect(() => {
    ...
    layers.forEach(id => {
      ...
      const volume = baseVol * layerGain * masterGainMultiplier * safetyMultiplier * masterAudioMultiplier * safeListeningCap * prioMultiplier * autoScaleMultiplier;
    ...
    if (subAudioRef.current) {
      ...
      const volume = s.volume * layerGain * masterGainMultiplier * safetyMultiplier * masterAudioMultiplier * safeListeningCap * prioSub * autoScaleMultiplier;
  ...
  }, [settings.audioTools.gainDb, settings.mainVolume, settings.globalModes, settings.subliminal, settings.advancedAudio, safetyMultiplier, settings.syncLab, isPlaying, settings.hzAutoScaling]);
  ...
  useEffect(() => {
    ...
    const onEnded = () => {
      ...
      if (shouldLoopAll) {
        // If it's a single track play (no track next), or if we are at the end of the list
        const isLastTrack = currentTrackIndex === currentPlaybackList.length - 1;
        
        if (isLastTrack || currentPlaybackList.length <= 1) {
          // Restart playlist or single track
          if (currentPlaybackList.length > 1) {
            playNextRef.current(true); // This will loop to start if it handles loop properly
          } else {
