...
export class ChunkManager {
  ...
  static async renderChunk(playlistId: string, chunkIndex: number, trackIds: string[], volume: number = 1.0, gainDb: number = 0, retryCount = 0): Promise<{ blob: Blob; duration: number } | null> {
    try {
      ...
      if (retryCount < 1) {
        ...
        return this.renderChunk(playlistId, chunkIndex, trackIds, volume, gainDb, retryCount + 1);
      }
      return null;
    }
  }
